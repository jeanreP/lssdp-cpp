...
/** Internal Function **/
static int send_multicast_data(const char* multicast_address, const char * data, const struct lssdp_interface intf, unsigned short ssdp_port);
...
static struct {
    ...
    const char * ADDR_MULTICAST_DEFAULT;
    ...
    .ADDR_MULTICAST_DEFAULT = "239.255.255.250",
...
int lssdp_socket_create(lssdp_ctx * lssdp) {
    ...
    if (strlen(lssdp->multicast_ip) != 0)
    {
        multicast_address.s_addr = inet_addr(lssdp->multicast_ip);
    }
    else
    {
        multicast_address.s_addr = inet_addr(Global.ADDR_MULTICAST_DEFAULT);
    }
    
    // set IP_ADD_MEMBERSHIP
    struct ip_mreq imr = {
        .imr_multiaddr.s_addr = multicast_address.s_addr,
...
int lssdp_send_msearch(lssdp_ctx * lssdp) {
    ...
    const char* multicast_addr = Global.ADDR_MULTICAST_DEFAULT;
    if (strlen(lssdp->multicast_ip) != 0)
    {
        multicast_addr = &lssdp->multicast_ip[0];
    }
    ...
    snprintf(msearch, sizeof(msearch),
        ...
        multicast_addr, lssdp->port,        // HOST
    ...
    for (i = 0; i < lssdp->interface_num; i++) {
        ...
        // send M-SEARCH
        int ret = send_multicast_data(multicast_addr, msearch, *intf, lssdp->port);
...
int lssdp_send_notify(lssdp_ctx * lssdp, const char* message_kind) {
    if (lssdp == NULL) {
        lssdp_error("lssdp should not be NULL\n");
        return -1;
    }
    ...
    for (i = 0; i < lssdp->interface_num; i++) {
        ...
        // send NOTIFY
        int ret = send_multicast_data(multicast_addr, notify, *intf, lssdp->port);
        if (ret == 0 && lssdp->debug) {
...
int lssdp_send_alive(lssdp_ctx * lssdp) {
    return lssdp_send_notify(lssdp, "alive");
}

int lssdp_send_byebye(lssdp_ctx * lssdp) {
    return lssdp_send_notify(lssdp, "byebye");
}
...
static int send_multicast_data(const char* multiaddress, const char * data, const struct lssdp_interface intf, unsigned short ssdp_port) {
    if (data == NULL) {
...
#ifdef WIN32
    if (InetPton(AF_INET, multiaddress, &dest_addr.sin_addr) == 0) {
        lssdp_error("inet_aton failed, errno = %s (%d)\n", strerror(errno), errno);
        goto end;
    }
#else
    if (inet_aton(multiaddress, &dest_addr.sin_addr) == 0) {
        lssdp_error("inet_aton failed, errno = %s (%d)\n", strerror(errno), errno);
        goto end;
    }
#endif
