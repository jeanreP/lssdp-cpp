...
typedef struct lssdp_ctx {
    ...
    char            multicast_ip[LSSDP_IP_LEN];             // SSDP address to use if empty default is used
    ...
    bool            send_response;                          // send response
...
/*
 * 06. lssdp_send_notify with alive message
 *
 * send SSDP NOTIFY packet to multicast address given in struct
 *
 * Note:
 *  - SSDP port must be setup ready before call this function. (lssdp.port > 0)
 *
...
/*
 * 06. lssdp_send_notify with byebye 
 *
 * send SSDP NOTIFY packet to multicast address given in struct
 *
 * Note:
